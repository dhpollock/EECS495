...
class NN:
	...
	def computeOutput(self, myInput):
		...
		for i in range(1, len(self.layers)):
			for j in range(len(self.layers[i])):
				self.layers[i][j].value = self.layers[i][j].bias*1
				for k in range(len(self.layers[i][j].weights)):
					self.layers[i][j].value = self.layers[i][j].value + self.layers[i][j].weights[k]*self.layers[i-1][k].value
		...
		while(sse > targetSSE and interCount < maxIter):
			...
			for i in range(len(myInput)):
				...
				for j in range(len(curOutput)):
						...
						self.layers[self.outputIndex][j].delta = float(curOutput[j])*(1 - float(curOutput[j]))*(float(myTarget[i][j]) - float(curOutput[j]))
						##Update Weight for output Neurons
						# self.layers[self.outputIndex][j].bias = self.layers[self.outputIndex][j].bias + lr*self.layers[self.outputIndex][j].delta
						# for k in range(len(self.layers[self.outputIndex][j].weights)):
						# 	self.layers[self.outputIndex][j].weights[k] = self.layers[self.outputIndex][j].weights[k] + lr*self.layers[self.outputIndex][j].delta*self.layers[self.outputIndex-1][k].value
				...
				for l in range(self.outputIndex-1, 0, -1):
					...
					for j in range(len(self.layers[l])):
						...
						for k in range(len(self.layers[l+1])):
							deltaSum = deltaSum + self.layers[l+1][k].delta*self.layers[l+1][k].weights[j]
							# print deltaSum
					...
					# 		self.layers[l][j].weights[k] = self.layers[l][j].weights[k] + lr*self.layers[l][j].delta*self.layers[l-1][k].value
				...
				for l in range(self.outputIndex-1, 0, -1):
					for j in range(len(self.layers[l])):
						...
						deltaW = 0
						for k in range(len(self.layers[l][j].weights)):
							deltaW = deltaW + lr*self.layers[l][j].delta*self.layers[l-1][k].value
						self.layers[l][j].weights[k] = self.layers[l][j].weights[k] + deltaW
				# 		if(printHelp):
			...
			for i in range(len(myInput)):
				curOutput = self.computeOutput(myInput[i])
				for j in range(len(curOutput)):
					tempSSE = tempSSE + (myTarget[i][j] - curOutput[j])*(myTarget[i][j] - curOutput[j])
			sse = tempSSE
...
class BPPerceptron:
	...
	def __init__(self, connections):
		self.value = 0
		self.weights = []
		self.bias = .01 + .05*(.5-np.random.random())

		for i in range(connections):
			self.weights.append(.01 + .05*(.5-np.random.random()))